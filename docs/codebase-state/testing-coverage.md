...
Testing is concentrated in the pure kernel layer, which has strong unit and property-based coverage. The runtime, adapters, and end-to-end integration paths have no automated tests. This document catalogs what is tested, what is not, and the planned approach for closing the gaps.
...
| Worker WS handler | No tests for connection, idle loop, job dispatch, re-registration, disconnect | High — complex async flow with `select!` races |
| Chat completions handler | No tests for HTTP request parsing, SSE output formatting, stream registration | High — client-facing contract |
| Relay (`relay_job`) | No tests for chunk forwarding, heartbeat sending, malformed message handling, `RelayOutcome` mapping | High — data path for all streaming |
| End-to-end integration | No tests combining HTTP client + gateway + mock worker | High — the assembled system has never been tested as a unit |
...
### Runtime and adapters: not yet tested

The runtime is intentionally simple ("so simple that its correctness is obvious by inspection"), but the wiring between kernel, registry, and effect executors has no automated verification. Adapter code (WS handler, HTTP handler, relay) contains async control flow with `select!` races, channel operations, and protocol parsing that can only be exercised by integration tests.

### Integration testing: planned

No integration tests exist. The planned approach is described below.
...
**Phase B** (core coverage):
5. Remaining correctness tests.
6. Remaining fault tolerance tests.
7. Leak detection tests.

**Phase C** (performance baseline):
8. Throughput and latency benchmarks.
9. Backpressure verification.
...
- No integration tests exist. All testing is at the kernel abstraction level.
...
- Runtime, adapters, relay, integration: **no tests**.
