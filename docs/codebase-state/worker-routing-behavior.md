...
- `GET /ws/worker` upgrades to websocket.
...
- Implemented (worker websocket registration + oneshot job dispatch + one-use ID flow + relay-based job handling with outcome mapping). Full worker protocol/event integration is complete. Chat completions handler not yet implemented (no real jobs flow through yet).
...
- Add worker handshake/version/capability validation and explicit registration acknowledgement message format.
- Wire dispatched jobs from chat request path into runtime so worker job channels receive real workload (requires chat completions handler).
